...
class MonoDataset(data.Dataset):
    ...
    def preprocess(self, inputs, color_aug, cutout_aug):
        ...
        for k in list(inputs):
            ...
            if "color" in k:
                ...
                if i == 0:
                    inputs[(n+ "_ori", im, i)] = self.midas_transform(np.array(f))
    ...
    def __getitem__(self, index):
        ...
        if do_cutout:
            cutout_aug = Cutout(mask_size=64)
        else:
            cutout_aug = (lambda x: x)
            
        self.preprocess(inputs, color_aug, cutout_aug)
...
class Cutout:
    ...
    def __call__(self, image):
        ...
        y1 = np.clip(y - self.mask_size // 2, 0, height)
        y2 = np.clip(y + self.mask_size // 2, 0, height)
        x1 = np.clip(x - self.mask_size // 2, 0, width)
        x2 = np.clip(x + self.mask_size // 2, 0, width)
        mask[y1:y2, x1:x2] = 0
